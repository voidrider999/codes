...
def draw_repeat_mode(length_diff, heading_diff, red_diff, green_diff, blue_diff):
...
def tab_repeat_mode(parent):
    ...
    label.pack()
    length_diff_sb = ttk.Spinbox(frame, from_=-100, to=100, increment=1)
    length_diff_sb.set(0)
    length_diff_sb.pack()

    label = ttk.Label(frame, text='Изменение поворота')
    label.pack()
    heading_diff_sb = ttk.Spinbox(frame, from_=-360, to=360, increment=5)
    heading_diff_sb.set(0)
    heading_diff_sb.pack()

    label = ttk.Label(frame, text='Изменение красного')
    label.pack()
    red_diff_sb = ttk.Spinbox(frame, from_=-255, to=255, increment=1)
    red_diff_sb.set(0)
    red_diff_sb.pack()

    label = ttk.Label(frame, text='Изменение зеленого')
    label.pack()
    green_diff_sb = ttk.Spinbox(frame, from_=-255, to=255, increment=1)
    green_diff_sb.set(0)
    green_diff_sb.pack()

    label = ttk.Label(frame, text='Изменение синего')
    label.pack()
    blue_diff_sb = ttk.Spinbox(frame, from_=-255, to=255, increment=1)
    blue_diff_sb.set(0)
    blue_diff_sb.pack()
    ...
    def on_draw_click():
        for _ in range(int(repeat_sb.get())):
            draw_repeat_mode(
                int(length_diff_sb.get()),
                int(heading_diff_sb.get()),
                int(red_diff_sb.get()),
                int(green_diff_sb.get()),
                int(blue_diff_sb.get()),
            )
